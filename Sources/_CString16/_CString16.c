...
// Calculates the number of UTF-8 bytes needed to encode a codepoint
...
// Decodes a codepoint from a UTF-8 string
...
static size_t encode_utf16(codepoint_t codepoint, utf16_t* utf16, size_t len, size_t index)
{
    ...
    utf16[index + 1] = low;
    // ___consoleLogInt(utf16[index]);
    // ___consoleLogInt(utf16[index + 1]);

    return 2;
}

// Converts a UTF-8 string to UTF-16
size_t utf8_to_utf16(utf8_t const* utf8, size_t utf8_len, utf16_t* utf16, size_t utf16_len, UTF16LengthResult length_result)
{
    ...
    switch (length_result) {
        case LengthInBytes:
            return utf16_index * sizeof(utf16_t);
            break;
        case LengthInCodepoints:
            return utf16_index;
            break;
    }
}

size_t utf8_to_utf16_len(utf8_t const* utf8, size_t utf8_len, UTF16LengthResult length_result)
{
    return utf8_to_utf16(utf8, utf8_len, NULL, 0, length_result);
}

size_t count_utf16_code_units(const utf16_t* utf16, size_t utf16_len) {
    size_t count = 0;

    for (size_t i = 0; i < utf16_len; ++i) {
        utf16_t code_unit = utf16[i];

        // Check if the current code unit is a high surrogate
        if ((code_unit >= 0xD800) && (code_unit <= 0xDBFF)) {
            // Check if there's a subsequent low surrogate
            if (i + 1 < utf16_len) {
                utf16_t next_code_unit = utf16[i + 1];
                if ((next_code_unit >= 0xDC00) && (next_code_unit <= 0xDFFF)) {
                    // This is a valid surrogate pair
                    ++count; // Count the high surrogate
                    ++count; // Count the low surrogate
        ...
        // For BMP characters and unmatched surrogates
        ++count;
    }
...
size_t count_characters_in_utf16_codepoints(const utf16_t* utf16, size_t utf16_len) {
    ...
    for (size_t i = 0; i < utf16_len; ++i) {
        ...
        if ((code_unit >= 0xD800) && (code_unit <= 0xDBFF)) {
            ...
            if (i + 1 < utf16_len) {
                ...
                if ((next_code_unit >= 0xDC00) && (next_code_unit <= 0xDFFF)) {
                    // This is a valid surrogate pair
                    ++character_count; // Count the surrogate pair as one character
                    ++i; // Skip the low surrogate
                    continue;
                }
            }
            // Unmatched high surrogate, count as individual character
            ++character_count;
        } else if ((code_unit >= 0xDC00) && (code_unit <= 0xDFFF)) {
            // Unmatched low surrogate, count as individual character
            ++character_count;
        } else {
...
utf16_t utf16_read_codepoint(utf16_t* utf16, size_t const offset) {
    return utf16[offset];
}
